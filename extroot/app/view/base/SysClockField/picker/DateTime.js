 ...
 *          
 */
Ext.define('Admin.view.base.SysClockField.picker.DateTime', {
	...
	renderTpl: [
		...
		'<div class="{baseCls}-header" style="display:none;">',
		...
		'</div>',
		'<table role="grid" id="{id}-eventEl" data-ref="eventEl" class="{baseCls}-inner" cellspacing="0" tabindex="0" aria-readonly="true" style="display:none;">',
		'<thead>',
		'<tr role="row">',
		'<tpl for="dayNames">',
		...
		// 指定时分秒渲染框架
		'<table id="{id}-timeEl" style="table-layout:auto;margin:0 auto;width:96%;text-align:center;" class="x-datepicker-inner" cellspacing="0">',
		'<tbody><tr>',
	...
	initDisabledDays: function() {
		...
		if (!me.disabledDatesRE && dd) {
			...
			for (d = 0; d < dLen; d++) {
				...
				re += Ext.isDate(dI) ? '^' + Ext.String
					.escapeRegex(Ext.Date.dateFormat(dI, me.format)) + '$' : dI;
				if (d !== len) {
					re += '|';
				}
			}
	...
	fullUpdate: function(date) {
		var me = this,
			...
			min = me.minDate ? eDate
			.clearTime(me.minDate, true) : Number.NEGATIVE_INFINITY,
			max = me.maxDate ? eDate.clearTime(
				me.maxDate, true) : Number.POSITIVE_INFINITY,
			ddMatch = me.disabledDatesRE,
			ddText = me.disabledDatesText,
			ddays = me.disabledDays ? me.disabledDays
			.join('') : false,
		...
		if (me.showToday) {
			...
			disableToday = (tempDate < min || tempDate > max || (ddMatch && format && ddMatch.test(eDate.dateFormat(
