Ext.define('Admin.view.alarms.alarmfilter.alarmCardQueryView', {
    ...
    viewModel: {
        stores: {
            ...
            cardlist_remote: {
                ...
                proxy: {
                    type: 'ajax',
                    url: '/alarm/alarmResource/getAllCards',
                    actionMethods: {
                        read: 'POST',
			...
			alarmlist_remote: {
                ...
                proxy: {
                    type: 'ajax',
                    url: '/alarm/alarmResource/getAllAlarmsByCard',
    ...
    controller: {
		...
		var treepanel =	this.getView().up().down('treepanel');
			...
			nejson= nejson +"neId :"+neId+","+"cardId :"+cardId+","+"hostName :"+hostName+","+"cardType :"+cardType+","+" cardName:"+cardName+","+" neType:"+neType+","+" ipAddress:"+ipAddress +"},"
				}
			...
			if(alarmRecords.length==0){
				...
				alarmjson=alarmjson.substring(0,alarmjson.length-1)+"]";
					 ...
					 Ext.Ajax.request({
                               url: 'alarm/alarmResource/checkAlarmCardFilter',
                               method: 'post',
                               params: {
							   ...
							   if(response.responseText=='true'){
								   Ext.Msg.confirm(_("Confirm"), _("Filter Operation Tips"), function (btn) {
									if(btn=='yes'){
									Ext.Ajax.request({
                                    url: 'alarm/alarmResource/addAlarmsByCard',
                                    method: 'post',
                                    params: {
										isCorbaFilter:isCorbaFilter,
										isUsed:isUsed,
		...
		onChecked:function(){
			...
			targetGrid.getStore().proxy.url = 'alarm/alarmResource/getAllAlarmsByCard';
        ...
        onQuery: function() {
           ...
           var cardgrid = this.getView().down('PagedGrid');	
		   var cardName = this.getView().down('textfield').getValue();
            cardgrid.getStore().proxy.url = 'alarm/alarmResource/getAllCards';
            cardgrid.getStore().proxy.extraParams = {cardName:cardName};
            cardgrid.getStore().reload();
