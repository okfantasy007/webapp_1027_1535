Ext.define('Admin.view.alarms.alarmfilter.alarmPortFilterView', {
    extend:'Ext.panel.Panel',
	xtype:'alarmPortFilterView',
	initComponent: function () {
		 this.portmodifiedRecoreds= new Ext.util.MixedCollection();
		 this.callParent();
	 },
	 requires: [
        'Admin.view.base.PagedGrid',
		'Admin.view.alarms.alarmfilter.alarmPortQueryView'
    ...
    viewModel: {
        stores: {
            ...
            filterlist_remote: {
                ...
                proxy: {
                    type: 'ajax',
                    url: '/alarm/alarmResource/getAlarmPortFilter',
					actionMethods : { 
					 read   : 'POST',
					}  
    ...
    controller: {  
		onDelete:function(){
			...
			var json= "[{"+"ircnenodeId :"+neid+"," +"isUsed :"+isUsed+"," +"isCorbaFilter :"+isCorbaFilter+"," +"isSaved :"+isSaved+"," +"alarmId :"+alarmId+"," +"alarmUrl :"+alarmUrl+","  +"alarmLevel :"+alarmLevel+"}]";
				...
				Ext.Ajax.request({
                                    url: 'alarm/alarmResource/deleteAlarmsByPort',
                                    method: 'post',
                                    params: {
                                        condition: json
                                    },
		...
		onQuery:function(){
            ...
            targetGrid.getStore().proxy.url = 'alarm/alarmResource/getAlarmPortFilter';
            targetGrid.getStore().proxy.extraParams ={keywords:keywords}
            targetGrid.getStore().reload();
		},
		
		onUsedChecked:function( me , rowIndex , checked , record , e , eOpts ) {
						this.getView().portmodifiedRecoreds.add(rowIndex,record);
						var length = record.childNodes.length;
        ...
        onAdd: function() {
            var targetpanel =  this.getView().down('treepanel');
			var alarmPortQueryView =  this.getView().down('alarmPortQueryView');
			this.getView().setActiveItem(alarmPortQueryView);
			 var  pageGrid =alarmPortQueryView.down('PagedGrid')
			 var alarmgrid = pageGrid.nextSibling().down('grid'); 
			 alarmPortQueryView.reset();
		...
		onSave:function(){
		    var targetGrid =  this.getView().down('treepanel');
			 ...
			 var len = view.portmodifiedRecoreds.items.length;
			 if(len==0){
				alarmjson ="[]";
				return;
			}else{
			 var alarmjson="[" ;
				...
				var record = view.portmodifiedRecoreds.items[i];
			...
			Ext.Ajax.request({
							url : 'alarm/alarmResource/modifyAlarmsByPort',
							method:'post',
							...
							targetGrid.getStore().reload();
							view.portmodifiedRecoreds.clear();
								}
		});
		},
        ...
        onRefresh: function() {
			...
			var keywords = this.getView().down('textfield').getValue();
            targetTreepanel.getStore().proxy.url = 'alarm/alarmResource/getAlarmPortFilter';
            targetTreepanel.getStore().proxy.extraParams ={keywords:keywords}
			targetTreepanel.getStore().reload();
			this.getView().portmodifiedRecoreds.clear();
        },
