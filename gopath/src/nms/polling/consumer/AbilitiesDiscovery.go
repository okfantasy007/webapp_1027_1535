...
func discoveryProcess(p *PollingTask, pResults *[]byte) {
	// results := (*p_results)
	// pingResult := *(results["icmpPing"].(*map[string]interface{}))

	// is_ping_ok := false

	// if status, exist := pingResult["ping_ok"].(bool); exist {
	// 	is_ping_ok = status
	// }

	pack := map[string]interface{}{
		"task_id": p.DiscoveryTaskId,
