...
router.post('/', function(req, res, next) {
	...
	var affirmPassword = req.body.password_again;
	// var nextLoginModifyPwd = req.body.change_password_next_login;
	...
	[
	    ...
	    function(conn,callback) {
	    	...
	    	var sql ="select * from sec_user where sec_user_id = "+userId;
	    ...
	    function(conn,callback) {
	    	var sql ="select count(*) as count  from sec_user where sec_user_id = "+userId +" and user_password = '"+md5(oldPassword)+"'";
	    	console.log('%%%%%%%%%%%%%%%%%',sql);
	    	conn.query(sql, function(err, rows, fields) {
	    		if(err){
	    			console.log('##########查询用户名 is err:##########'+err);
	    			...
	    			if(rows[0].count>0){
	    				...
	    				callback({errno: 40000,result:false, errmsg: 'your original password is error, please re-enter'}, conn);
	    			}
		...
		function(conn,rows,callback){	
			// var sql = '';
			// if(nextLoginModifyPwd==1){
			// 	sql = 'update sec_user_strategy  set change_password_next_login = 1,cannot_change_password=0';
			// }else{
			// 	sql = 'update sec_user_strategy  set change_password_next_login = 0';
			// }
			var sql ="update sec_user_strategy set change_password_next_login = '0' where sec_user_id = "+userId ;
			// vsr sql = 'update sec_user_strategy  set change_password_next_login = 0';
			conn.query(sql, function(err, result) {
				if(err){
					console.log('##########保存密码入历史库出错##########');
					callback({errno: 40101,result:false, errmsg: err}, conn);
				}else{
