...
// -------------------------------- 拓扑子网列表 -------------------------------------
router.get('/subnet_info_old', function(req, res, next) {
	var clause = '1=1';
	if (req.query.param != '') {
	...
	APP.dbpool.getConnection(function(err, conn) {
		var sql = sprintf("\
			SELECT count(*) as count \
			 ...
			 WHERE tms.map_parent_id != -1 and tms.res_type_name = 'TOPO_SUBNET' and tms.is_visible = 1 and tms.topo_type_id = vtt.topo_type_id and %s;\
...
router.get('/subnet_info', function(req, res, next) {
    var clause = '1=1';
	if (req.query.param != '') {
		clause = sprintf("(tms.symbol_id like '%%%s%%' or tms.symbol_name1 like '%%%s%%' or tms.remark like '%%%s%%' or vtt.display_name like '%%%s%%')", req.query.param, req.query.param, req.query.param, req.query.param);
	}
    ...
    async function subnet_info() {
        try{
			...
			SELECT count(*) as count \
		      FROM topo_symbol tms, topo_node_type vtt \
			 WHERE tms.map_parent_id != -1 and tms.res_type_name = 'TOPO_SUBNET' and tms.is_visible = 1 and tms.topo_type_id = vtt.topo_type_id and %s and %s;\
		    SELECT tms.symbol_id, tms.symbol_name1, tms.remark, vtt.display_name as topo_type_name \
              FROM topo_symbol tms, topo_node_type vtt \
             WHERE tms.map_parent_id != -1 and tms.res_type_name = 'TOPO_SUBNET' and tms.is_visible = 1 and tms.topo_type_id = vtt.topo_type_id and %s and %s \
             LIMIT %d, %d;\
            ...
            if (err) {
                await conn.rollback();
                res.json(200, {success: false, msg: 'get subnet info failed'});  
            } else {
                res.json(200, {success: true, msg: 'get subnet info success'});  
            }
        }   
    };
...
//导出全部
router.get('/subnet_info/exportAll/datacsv', function(req, res, next) {
    var clause = '1=1';
    if (req.query.param != '') {
      clause = sprintf("(tms.symbol_id like '%%%s%%' or tms.symbol_name1 like '%%%s%%' or tms.remark like '%%%s%%' or vtt.display_name like '%%%s%%')", req.query.param, req.query.param, req.query.param, req.query.param);
    }
    ...
    APP.dbpool.getConnection(function(err, conn) {
        var sql = sprintf("SELECT tms.symbol_id, tms.symbol_name1, tms.remark, vtt.display_name as topo_type_name \
                          FROM topo_symbol tms, topo_node_type vtt \
                          WHERE tms.map_parent_id != -1 and tms.res_type_name = 'TOPO_SUBNET' and tms.is_visible = 1 \
                          and tms.topo_type_id = vtt.topo_type_id and %s;",clause);

        conn.query(sql, function(err, rows, fields) {
            var fields = ['symbol_id','symbol_name1','remark', 'topo_type_name'];
            var fieldNames = [req.__('Symbol ID'), req.__('Node Name'),req.__('Remark'),req.__('Topo Type')];
            var m_data = [];
            for (var i in rows) {
                m_data.push(rows[i]);   
            }
            ...
            res.setHeader("Content-Disposition", "attachment; filename=" + "all_subnet_info.csv");
...
//导出当前页
router.get('/subnet_info/exportCurrentPage/datacsv', function(req, res, next) {
    var clause = '1=1';
    if (req.query.param != '') {
      clause = sprintf("(tms.symbol_id like '%%%s%%' or tms.symbol_name1 like '%%%s%%' or tms.remark like '%%%s%%' or vtt.display_name like '%%%s%%')", req.query.param, req.query.param, req.query.param, req.query.param);
    }
    ...
    APP.dbpool.getConnection(function(err, conn) {
       var sql = sprintf("SELECT tms.symbol_id, tms.symbol_name1, tms.remark, vtt.display_name as topo_type_name \
        ...
        conn.query(sql, function(err, rows, fields) {
            ...
            var newCsv = iconv.encode(csv, 'GBK'); // 转编码
            res.setHeader('Content-Type', 'application/octet-stream');
            res.setHeader("Content-Disposition", "attachment; filename=" + "currentPage_subnet_info.csv");
            res.status(200).send(newCsv);
        }); 
    });      
});
...
//导出选择项
router.get('/subnet_info/exportSelected/datacsv', function(req, res, next) {
    ...
    APP.dbpool.getConnection(function(err, conn) {
      var sql = sprintf("SELECT tms.symbol_id, tms.symbol_name1, tms.remark, vtt.display_name as topo_type_name \
        ...
        conn.query(sql, function(err, rows, fields) {
            ...
            var newCsv = iconv.encode(csv, 'GBK'); // 转编码
            res.setHeader('Content-Type', 'application/octet-stream');
            res.setHeader("Content-Disposition", "attachment; filename=" + "selected_subnet_info.csv");
            res.status(200).send(newCsv);
        }); 
    }); 
});
